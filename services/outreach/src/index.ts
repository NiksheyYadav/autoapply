...
import { loadOutreachServiceEnv } from './env.js';
import { createTransport } from './lib/transport.js';

async function main(): Promise<void> {
  const env = loadOutreachServiceEnv();
  const logger = createLogger({ service: 'outreach-service', level: env.LOG_LEVEL, pretty: env.LOG_PRETTY });
  ...
  const transport = createTransport({ driver: env.OUTREACH_TRANSPORT_DRIVER, logger });

  const app = buildApp({ db, sql, env, logger, broker, tokenVerifier, transport });
