...
import { loadReferralsServiceEnv } from './env.js';
import { registerConsumers } from './worker.js';

async function main(): Promise<void> {
  const env = loadReferralsServiceEnv();
  const logger = createLogger({ service: 'referrals-service', level: env.LOG_LEVEL, pretty: env.LOG_PRETTY });
