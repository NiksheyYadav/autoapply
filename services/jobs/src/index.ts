...
import { loadJobsServiceEnv } from './env.js';

async function main(): Promise<void> {
  const env = loadJobsServiceEnv();
  const logger = createLogger({ service: 'jobs-service', level: env.LOG_LEVEL, pretty: env.LOG_PRETTY });
